...
func MakeNode(buf []byte) (*Node, error) {
	...
	if err == nil {
		...
		for _, ele := range bsonNode {
			switch ele.Key {
			case "node_height":
				node.subtreeHeight = (int8)(ele.Value.(int32))
			case "node_size":
...
func (node *Node) encodedSizeEx(isTrackable bool) int {
	if node.isLeaf() && isTrackable {
		...
		bsonval = append(bsonval, bson.E{"node_height", node.subtreeHeight})
...
func (node *Node) writeBytesEx(w io.Writer, isTrackable bool) error {
	...
	if node.isLeaf() && isTrackable {
		...
		if cause != nil {
			return fmt.Errorf("writing bson, %w", cause)
		}
